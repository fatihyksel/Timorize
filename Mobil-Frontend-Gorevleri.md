...
**Görev**  
Kayıtlı kullanıcının sisteme giriş yapabilmesi için sayfa tasarımı ve implementasyonu.

### UI Bileşenleri
- Responsive giriş formu ( mobile uyumlu)
- Email input alanı (`type="email"`, `autocomplete="email"`)
...
##  Teknik Detaylar (Mobil)

- **Framework:** React Native (Expo)
- **UI:** Native Components + StyleSheet
- **Form Yönetimi:** useState + basit validasyon
- **State Yönetimi:** useState / useEffect (gerekirse Context API)
- **Navigasyon:** React Navigation (Stack / Tab, giriş sonrası dashboard)
- **API:** Fetch API + merkezi API servis dosyası
- **Auth:** JWT + AsyncStorage (gerekirse SecureStore)
- **Performans:** FlatList optimizasyonu
- **Erişilebilirlik:** accessibilityLabel + temel WCAG uyumu
